...
ВНИМАНИЕ: описанное ниже адаптивное правило ЗАМЕНЕНО. Замер 2026-08-05
(`score_separation.py`) показал, что излома в распределении косинуса нет
вовсе -- детектор обрыва срабатывал в диапазоне 20-43 при защитном
минимуме 20, то есть всегда упирался в пол. Фактически это фиксированное K.
Итог: 70.7% полноты при 39.9% точности и 138 документах на объект.

Работающее правило -- оценка LLM по словесной рубрике 0-10 с единым
порогом 7.5: 76.5% полноты при 93.3% точности и 44 документах. Косинус и
логит кросс-энкодера -- меры сходства без опоры, между объектами
несравнимы, поэтому единого порога у них нет в принципе; рубрика задаёт
шкалу словами и потому одинакова для всех объектов. Подтверждение: Opus и
Sonnet независимо сошлись на 7.5.
Подробно: docs/architecture/retrieval_research_report.md, раздел 5.

Прежнее обоснование сохранено ниже как история решения.
...
Значения по умолчанию (50 / 200 / 3) выбраны перебором, а не на глаз:
они дают 70.7% полноты при 138 документах и 39.9% точности -- нижняя
граница цели при наименьшем объёме. Отключить детектор обрыва совсем
(`--min-ratio` очень большой) даёт 90.1%, но ценой 514 документов и 14.1%
точности, то есть это просто «взять всё».
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--min-keep", type=int, default=50,
                    help="защитный минимум: проходит независимо от оценок. 50 -- "
                         "точка перегиба по перебору: с 20 до 50 полнота растёт на "
                         "10.9 пункта, с 50 до 100 -- всего на 2.8 при потере пяти "
                         "пунктов точности")
    ap.add_argument("--max-keep", type=int, default=200,
                    help="потолок набора. Не связывающее ограничение: 200, 300 и "
                         "500 дают одно и то же, детектор обрыва срабатывает "
                         "раньше двухсот почти всегда")
