...
- [~] 6.7 **Код написан, деплой намеренно не выполняется.** Найдено
      ...
      (`test_parsers360_ingest_v5.py`); suite 85/85. **Не запускался и не
      будет запускаться на сервере в рамках этого change** — по явной
      просьбе. Не отмечено `[x]`: реальный вызов API/запись в БД не
      проверены (не должны быть, деплой не входит в скоуп).
