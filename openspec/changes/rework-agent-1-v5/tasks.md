...
- [ ] 6.7 **(найдено 2026-07-23, осознанно отложено)** `parsers360_ingest.py`
      до сих пор пишет в старую схему `agent_1.raw_items`, а не в
      `agent_1_v5.raw_posts` — design.md (Migration Plan, шаг 3) и
      proposal.md называли это частью change, но задача никогда не попала
      в чеклист. Без переключения `agent_1_v5` — статичный снимок на дату
      миграции (2026-07-20), новые новости в v5-пайплайн не попадают.
      Подтверждено (2026-07-23): крон сейчас **не запущен** ни в каком виде
      («просто скрипты», по словам пользователя) — растущего разрыва нет,
      catch-up-миграция при переключении не понадобится. Пользователь явно
      попросил не разворачивать это на постоянку сейчас — задача остаётся
      открытой намеренно, не блокирует archive этого change. Логичный
      следующий шаг (отдельным change или отдельной задачей): переписать
      `parsers360_ingest.py` на запись в `source`/`raw_posts` (маппинг полей
      уже решён в `migrate_v5.py::resolve_name_source`/`insert_raw_post` —
      прямое переиспользование), без постановки на cron/systemd, пока не
      попросят явно.
