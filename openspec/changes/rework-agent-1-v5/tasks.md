...
- [ ] 3.1 cProfile «до» (100 документов, **реальный корпус, не
      синтетика**) — зафиксировать базовую линию, подтвердить долю I/O.
...
- [ ] 3.3 MinHash: заменить схему хэш-миксинга `(a*x+b) mod (2⁶¹−1)` на
      ...
      broadcast `(128, N)` + `min(axis=1)` (см. design.md D5 — «просто
      numpy» на старой формуле почти не ускоряет из-за переполнения
      `uint64` при `a*x`, вынуждающего object-dtype). Заменить `blake2b`
      на быстрый некриптографический хэш при конструировании шинглов —
      после векторизации сигнатуры именно хэширование шинглов становится
      узким местом. Пороги/шинглование/бэнды (D3) — без изменений;
      accept/reject остаётся точным Jaccard по множествам шинглов, смена
