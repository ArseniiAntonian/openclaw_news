...
`label_kr_worker` MUST сделать один общий LLM-вызов (`agent_1`, шаг `relevance`) для документов, у которых остался хотя бы один source-allowed
...
`evidence` (каждый фрагмент — грaunded в тексте), `reasoning_steps`,
...
Каждый LLM-шаг (`relevance`, `impact`, `sber_paid_news`, `entity_tonality`) MUST быть независимо чекпоинтирован в
