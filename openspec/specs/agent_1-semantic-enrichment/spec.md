# agent_1-semantic-enrichment Specification

## Purpose

Извлечение сущностей и событий, groundedness-валидация
(`extract_semantics_worker.py`). Снимок «как есть» на 2026-07-17 (change
`document-agent-1-pipeline`).

> В архитектуре v5 эта капабилити уходит из Агента 1 (переезд в Агент 4:
> GLiNER + драйверный слой) — см. change `rework-agent-1-v5`.

## Requirements

### Requirement: Триггер semantic-enrichment и claim job'ов
Каждый `clean_items.id` с хотя бы одной строкой `document_kr_labels` MUST
порождать одну строку `processing_jobs` с
...
состояние очереди или персистентность — детерминированные задачи остаются
за Python-слоем, персистентность — за самим воркером.
